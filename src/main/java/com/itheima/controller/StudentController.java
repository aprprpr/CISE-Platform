...
public class StudentController extends BaseController {
    ...
    // @RequestMapping({ "applylist1"})
    // public String queryAllProject(HttpServletRequest request, Integer project_id,
    // String project_name, String teacher_name, @RequestParam(required = false,
    // defaultValue = "1", value = "pn") Integer pn){
    ...
    public ResponseResult<Void> save(Apply apply, RedirectAttributes attributes, HttpSession session) {
        Integer sid = Integer.valueOf(session.getAttribute("studentid").toString());
        // 设置学生ID到apply对象中
        apply.setStudentId(sid);
        // 检查该学生是否已经申请过该项目（使用学生ID + 项目ID）
        Apply result = applyDao.selectByStudentAndProject(sid, apply.getProjectId());
        if (result != null) {
    ...
    public String update(Student student, MultipartFile img) throws IOException {
        ...
        log.debug("姓名:{},dian电话号码:{},性别:{},", student.getStudentName(), student.getStudentPhonenumber(),
