...
public class AdjuController {
    ...
    public String applylist(Model model, HttpSession session){
        ...
        java.util.List<com.itheima.domain.Judge> judges = adjuService.queryAllNeedGradeAdju(adjuId);
    ...
    public String score(Model model, Integer studentId, Integer projectId, HttpSession session){
        Integer adjuId=Integer.valueOf(session.getAttribute("adjuId").toString());
        model.addAttribute("adjuId",adjuId);
        model.addAttribute("judge", adjuService.querySingleNeedGradeAdju(studentId, projectId, adjuId));
        return "adju/score";}
