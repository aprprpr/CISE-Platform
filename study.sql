...
CREATE TABLE `evaluation_score`  (
  ...
  `evaluator_id` int(11) NOT NULL COMMENT '评分人ID',
  `evaluatee_id` int(11) NOT NULL COMMENT '被评人ID（可以是项目或个人）',
  `score_type` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '评分类型：自评/团队互评/导师评价/专家评价',
  `score_value` decimal(5, 2) NOT NULL COMMENT '总分或分项分',
  `score_detail` json COMMENT 'JSON存储分项得分，如创新性、完成度等',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `idx_project_id`(`project_id`) USING BTREE,
  INDEX `idx_evaluator_id`(`evaluator_id`) USING BTREE,
  INDEX `idx_evaluatee_id`(`evaluatee_id`) USING BTREE,
  INDEX `idx_score_type`(`score_type`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC COMMENT = '评分记录表';

-- ----------------------------
-- Table structure for evaluation_weight
-- ----------------------------
DROP TABLE IF EXISTS `evaluation_weight`;
CREATE TABLE `evaluation_weight`  (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '权重ID',
  `weight_name` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '权重名称',
  `weight_value` decimal(5, 2) NOT NULL COMMENT '权重值（小数）',
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC COMMENT = '评价维度权重配置表';

-- ----------------------------
-- Table structure for project_final_score
-- ----------------------------
DROP TABLE IF EXISTS `project_final_score`;
CREATE TABLE `project_final_score`  (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `project_id` int(11) NOT NULL COMMENT '项目ID',
  `final_score` decimal(5, 2) NOT NULL COMMENT '最终综合得分',
  `score_detail` json COMMENT '得分明细（JSON）',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `idx_project_id`(`project_id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC COMMENT = '项目最终得分表';
